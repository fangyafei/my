...
//         echo "<br>";
    ...
    function _count($params){
    	...
    	$sql = "SELECT COUNT(1) AS count FROM {$tables}{$conditions}";
    ...
    function add($data, $compatible = false)
    {
        ...
        //echo "<br>{$mode} INTO {$this->table}{$insert_info['fields']} VALUES{$insert_info['values']}";
    ...
    function exec($sql){
    	if($rs = $this->db->query($sql, $this->db->_link_id))
    	{
    		return $rs;
    	}
    	else
    	{
    		@trigger_error($sql.':'.$this->db->error(),E_USER_WARNING);
    		return false;
    	}
    }
     ...
     * @return boolean
     */
    public function setInc($conditions,$field,$step=1) {
    	$conditions  = $this->_getConditions($conditions, false);
    	$edit_fields = $field.'='.$field.'+'.$step;
    	$this->db->query("UPDATE {$this->table} SET {$edit_fields}{$conditions}");
    	return $this->db->affected_rows();
    }
 ...
 /**
     ...
     * @return boolean
     */
    public function setDec($conditions,$field,$step=1) {
       $conditions  = $this->_getConditions($conditions, false);
       $conditions .= ' and '.$field.' > 0 ';
       $edit_fields = $field.'='.$field.'-'.$step;
       $this->db->query("UPDATE {$this->table} SET {$edit_fields}{$conditions}");
    	return $this->db->affected_rows();
    }
