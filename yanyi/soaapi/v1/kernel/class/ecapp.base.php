...
EOT;
    ...
    function _build_upload($params = array())
    {
        ...
        $upload_url = isset($params['upload_url']) ? $params['upload_url'] : 'index.php?app=images&act=upload';
...
class BaseVisitor 
{
    ...
    function _get_detail()
    {
        ...
        $member_info = $model_member->findAll(array(
            ...
            'fields'        => 'member.email, money, frozen, password, real_name, logins, portrait, store_id, store.state, sgrade , idserve,serve.state servestate,serve.serve_type,serve.idserve',
    ...
    function setDef($key, $val){
    	$model_member =& m('member');
    	if($this->has_login){
    		$model_member->edit($this->info["user_id"], array($key=>$val));
    	}
